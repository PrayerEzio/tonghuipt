...
// 		$this->log('baifubao query pay result by order_no success');
		...
		if (SP_PAY_RESULT_WAITING == $order_state) {
			$this->log('the order state is right, the order is waiting for pay');
			return true;
		} elseif (SP_PAY_RESULT_SUCCESS == $order_state) {
	 ...
	 */
	function notify_bfb() {
		$rep_str = "<html><head>" . BFB_NOTIFY_META .
	...
	function query_baifubao_pay_result_by_order_no($order_no) {
		$params = array (
			'service_code' => BFB_QUERY_INTERFACE_SERVICE_ID, // 查询接口的服务ID号
			'sp_no' => SP_NO,
			'order_no' => $order_no,
			'output_type' => BFB_INTERFACE_OUTPUT_FORMAT, // 百度钱包返回XML格式的结果
			'output_charset' => BFB_INTERFACE_ENCODING, // 百度钱包返回GBK编码的结果
			'version' => BFB_INTERFACE_VERSION,
		...
		$query_url = BFB_QUERY_ORDER_URL . '?' . $params_str;
		$this->log(
				sprintf('the url of query baifubao pay result is [%s]', 
						$query_url));
		$content = $this->request($query_url);
		$retry = 0;
		while (empty($content) && $retry < BFB_QUERY_RETRY_TIME) {
			$content = $this->request($query_url);
			$retry++;
		}
		if (empty($content)) {
		...
		if (BFB_PAY_RESULT_SUCCESS != $response_arr ['pay_result']) {
			$this->log(
					sprintf(
	...
	private function make_sign($params) {
		if (is_array($params)) {
			...
			if (ksort($params)) {
				...
				// 选择相应的加密算法
				if ($params ['sign_method'] == SIGN_METHOD_MD5) {
					return md5($sign_str);
				} else if ($params ['sign_method'] == SIGN_METHOD_SHA1) {
